...
let
  ...
  # The template must be an unmodified Bun release executable: a Nix-patched
  # template corrupts under bun >= 1.3.14 (oven-sh/bun#31023). omp 18.2.x
  # embeds standalone bytecode, and bytecode generated by bun 1.3.x fails to
  # load under the matching runtime for omp's bundle, so the pinned 1.4.2
  # release executable doubles as the compiler of the standalone binary
  # (upstream requires and builds with bun >= 1.4).
  bunRuntimeVersion = "1.4.2";
  bunRuntimeTemplate = stdenvNoCC.mkDerivation {
    ...
    dontConfigure = true;
    dontBuild = true;
    # The template doubles as the compiler; fixup would alter the PT_LOAD layout.
    dontFixup = true;
...
stdenv.mkDerivation {
  ...
  buildPhase = ''
    ...
    # Compile with the template's own Bun release so the embedded bytecode
    # matches the runtime; on Linux its PT_INTERP points at /lib64, so run it
    # through the Nix loader.
    echo "Compiling standalone binary..."
    (cd packages/coding-agent && ${lib.optionalString stdenv.hostPlatform.isLinux ''
      "${stdenv.cc.bintools.dynamicLinker}" --library-path "${
        lib.makeLibraryPath [
          stdenv.cc.libc
          stdenv.cc.cc.lib
        ]
      }" \
    ''}"${bunRuntimeTemplate}/libexec/bun" ${./compile-standalone.ts} "${bunRuntimeTemplate}/libexec/bun")
